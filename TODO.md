...
- Move `src/server/mod.rs` test module to the end (or move items before it) to satisfy `items_after_test_module`.
- Re-run full gate: `RUSTFLAGS='-D warnings' cargo build --all-targets --all-features && RUSTFLAGS='-D warnings' cargo test --all-targets --all-features && cargo clippy --all-targets --all-features -- -D warnings`.
