...
fn install_fake_redis_benchmark(path: &Path) -> Result<()> {
...
if [[ "${args}" == *" PING "* ]]; then
  ...
  if ((attempts == 1)); then
    echo "${REDIS_CLI_PING_FAIL_MESSAGE:-Could not connect to Redis at 127.0.0.1:6379: Connection refused}" >&2
    exit 1
  fi
...
#[test]
fn benchmark_profile_retries_transient_connectivity_close() -> Result<()> {
    let nanos = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
    let label = format!("metadata-preflight-close-retry-{nanos}");
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let script_path = repo_root.join("scripts/benchmark_profile.sh");
    let fake_bin_dir = env::temp_dir().join(format!("ralphdb-fake-cli-close-retry-bin-{nanos}"));
    fs::create_dir_all(&fake_bin_dir)?;
    install_fake_success_redis_benchmark(&fake_bin_dir)?;
    install_fake_flaky_redis_cli(&fake_bin_dir)?;
    let ping_state_file = fake_bin_dir.join("redis-cli-ping-attempts.txt");
    ...
    assert!(
        status.success(),
        "benchmark profile should retry one transient redis-cli preflight closed-connection error and succeed"
    );
    ...
    assert_eq!(
        ping_attempts.trim(),
        "2",
        "redis-cli preflight PING should run twice when the first attempt reports a transient connection close"
    );
...
#[test]
fn benchmark_profile_parses_comma_delimited_mixes_and_modes() -> Result<()> {
    let nanos = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
    let label = format!("metadata-comma-parse-{nanos}");
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let script_path = repo_root.join("scripts/benchmark_profile.sh");
    let fake_bin_dir = env::temp_dir().join(format!("ralphdb-fake-comma-parse-bin-{nanos}"));
    fs::create_dir_all(&fake_bin_dir)?;
    install_fake_success_redis_benchmark(&fake_bin_dir)?;
    install_fake_flaky_redis_cli(&fake_bin_dir)?;
    let ping_state_file = fake_bin_dir.join("redis-cli-ping-attempts.txt");
    ...
    let status = Command::new("bash")
        ...
        .env("REPEATS", "1")
        .env("MIXES", "1:1, 32:8")
        .env("MODES", "basic,mset")
        .env("BENCH_TIMEOUT_SECONDS", "0")
        .status()?;

    assert!(
        status.success(),
        "benchmark profile should parse comma-delimited mixes/modes and complete when synthetic benchmark commands succeed"
    );
    ...
    assert_eq!(metadata.get("mixes").map(String::as_str), Some("1:1 32:8"));
    assert_eq!(
        metadata.get("modes").map(String::as_str),
        Some("basic mset")
    );
...
#[test]
fn benchmark_profile_records_run_failure_context() -> Result<()> {
    if !command_exists("redis-cli") || !command_exists("redis-benchmark") {
        eprintln!(
            "skipping benchmark metadata integration test because redis-cli/redis-benchmark are unavailable"
        );
    ...
    let nanos = SystemTime::now().duration_since(UNIX_EPOCH)?.as_nanos();
    let label = format!("metadata-run-failure-{nanos}");
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let script_path = repo_root.join("scripts/benchmark_profile.sh");
    let fake_bin_dir = env::temp_dir().join(format!("ralphdb-fake-bin-{nanos}"));
    fs::create_dir_all(&fake_bin_dir)?;
    ...
    assert!(
        !status.success(),
        "benchmark profile should fail when a benchmark run exits non-zero"
    );
    ...
    assert!(
        ...
        "last_run_started_output_file should point to the failed run output"
    );
    assert_eq!(
        metadata
            .get("last_run_completed_output_file")
